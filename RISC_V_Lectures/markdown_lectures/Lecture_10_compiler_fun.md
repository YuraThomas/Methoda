...
В статье по прошивке нашего risc-v команды писались побитово, чтобы
...
По плану в статье будет сначала перевод с языка C на ассемблер, затем
напишем обязательные файлы для работы программ на написанном самостооятельно
процессоре, далее посмотрим на симуляцию работы программы в modelsim.
Эта статья во многом основана на лабораторной работе \№14 курса "Архитектуры
процессорных систем" института МПСУ 
(ссылка: https://github.com/MPSU/APS/tree/master/Labs/14.%20Programming).
Добавлены примеры команд компилятору и результаты их выполнения.

Процессор из лабораторных работ отличается от нашего наличием регистров
контроля статуса и обработчика прерываний. Название такого набора
команд - rv32i_zicsr. I - это обязательные команды; zicsr - как раз
расширение, добавляющее работу с упомянутым железом. В нашем случае набор
инструкций будет просто rv32i.
...
в лабораторной из того же курса АПС. Ссылка для установки (около 0,5 ГБ):
