...
Оптимизирует схему (netlist), модет убрать некоторые элементы/заменить их на другие
...
**Добавление ```via``` (линии питания и земли, к которым ячейки будут подключены)**
